...
def get_bomb_plant_df(parser: DemoParser):
    ...
    tick_df = parser.parse_ticks(
        ["total_rounds_played", "last_place_name", "game_time"], ticks=df["tick"]
    )

    return pd.merge(df, tick_df, how="inner", on=["name", "tick"])[
        ["total_rounds_played", "last_place_name", "name", "game_time"]
    ]
...
def get_bomb_plant_site(df: pd.DataFrame, round: int):
    v = list(
        filter(
            lambda x: x is not None and x.startswith("Bombsite"),
            df.loc[df["total_rounds_played"] == round]["last_place_name"].values,
        )
